...
sr = NaN;     % s/r = metti NaN nel caso SR/IR, sennò cambia sr sotto 
pw = 1;      % pwt attivo SR/IR: 1 o 2. DR: valore ignorato.
conf = 'SR';

inpath  = 'E:\APC_Prove\20x13\29_04_26';
outpath = 'E:\APC_Prove\20x13\Analisi\database_29_04_26_pt2\';
...
cases.filename = [
    "APC_20x13EP_SR_RPM1000.csv"
    "APC_20x13EP_SR_RPM1500.csv"
    "APC_20x13EP_SR_RPM2000.csv"
    "APC_20x13EP_SR_RPM2500.csv"
    "APC_20x13EP_SR_RPM3000.csv"
    "APC_20x13EP_SR_RPM3500.csv"
    "APC_20x13EP_SR_RPM4500.csv"
    "APC_20x13EP_SR_RPM4500_2.csv"
];
...
tareRPMmax = 50;          % [rpm] soglia motore spento
tarePWMmax = 1100;        % [us] modifica se il tuo minimo non è 1100
...
col.A1   = 6;
col.rpm1 = 7;    % [RPM]
% Potenze lette direttamente dal CSV
col.Pelec1 = 8;
col.Pmech1 = 9;  
...
for ic = 1:height(cases)
    ...
    %% ===================== START/END ZERO DIAGNOSTIC ===============

    idxStart1s = t >= t(1) & t <= t(1) + 1.0;
    idxEnd1s   = t >= t(end) - 1.0 & t <= t(end);
    
    T1_all   = getColumn(data, col.T1);
    T2_all   = getColumn(data, col.T2);
    RPM1_all = getColumn(data, col.rpm1);
    RPM2_all = getColumn(data, col.rpm2);
    PWM1_all = getColumn(data, col.pwm1);
    PWM2_all = getColumn(data, col.pwm2);
    
    fprintf('\nZERO diagnostic for %s\n', filename);
    
    fprintf('  START 1s | PWM1 min/mean/max = %.1f / %.1f / %.1f | RPM1 min/mean/max = %.1f / %.1f / %.1f | T1 mean = %.5f kgf\n', ...
        min(PWM1_all(idxStart1s),[],'omitnan'), mean(PWM1_all(idxStart1s),'omitnan'), max(PWM1_all(idxStart1s),[],'omitnan'), ...
        min(RPM1_all(idxStart1s),[],'omitnan'), mean(RPM1_all(idxStart1s),'omitnan'), max(RPM1_all(idxStart1s),[],'omitnan'), ...
        mean(T1_all(idxStart1s),'omitnan'));
    
    fprintf('  START 1s | PWM2 min/mean/max = %.1f / %.1f / %.1f | RPM2 min/mean/max = %.1f / %.1f / %.1f | T2 mean = %.5f kgf\n', ...
        min(PWM2_all(idxStart1s),[],'omitnan'), mean(PWM2_all(idxStart1s),'omitnan'), max(PWM2_all(idxStart1s),[],'omitnan'), ...
        min(RPM2_all(idxStart1s),[],'omitnan'), mean(RPM2_all(idxStart1s),'omitnan'), max(RPM2_all(idxStart1s),[],'omitnan'), ...
        mean(T2_all(idxStart1s),'omitnan'));
    
    fprintf('  END 1s   | PWM1 min/mean/max = %.1f / %.1f / %.1f | RPM1 min/mean/max = %.1f / %.1f / %.1f | T1 mean = %.5f kgf\n', ...
        min(PWM1_all(idxEnd1s),[],'omitnan'), mean(PWM1_all(idxEnd1s),'omitnan'), max(PWM1_all(idxEnd1s),[],'omitnan'), ...
        min(RPM1_all(idxEnd1s),[],'omitnan'), mean(RPM1_all(idxEnd1s),'omitnan'), max(RPM1_all(idxEnd1s),[],'omitnan'), ...
        mean(T1_all(idxEnd1s),'omitnan'));
    
    fprintf('  END 1s   | PWM2 min/mean/max = %.1f / %.1f / %.1f | RPM2 min/mean/max = %.1f / %.1f / %.1f | T2 mean = %.5f kgf\n', ...
        min(PWM2_all(idxEnd1s),[],'omitnan'), mean(PWM2_all(idxEnd1s),'omitnan'), max(PWM2_all(idxEnd1s),[],'omitnan'), ...
        min(RPM2_all(idxEnd1s),[],'omitnan'), mean(RPM2_all(idxEnd1s),'omitnan'), max(RPM2_all(idxEnd1s),[],'omitnan'), ...
        mean(T2_all(idxEnd1s),'omitnan'));
    ...
    if computeRotor1
            ...
            R1 = rotorStatsSingleRamp( ...
                ...
                col.pwm1, col.Pmech1, col.Pelec1, ...
                rpmTarget_R1, NaN, rho, D, kgf2N, ...
                applyTareCorrection, tareWindow_s, tareGuard_s, ...
                tareRPMmax, tarePWMmax, tareUseEndZero, ...
    ...
    if computeRotor2
       ...
       R2 = rotorStatsSingleRamp( ...
                ...
                col.pwm2, col.Pmech2, col.Pelec2, ...
    ...
    row.P_mech_csv_total_W = NaN;
    row.P_elec_csv_total_W = NaN;
    ...
    if nRotors == 2
        ...
        row.P_mech_csv_total_W = row.P1_mech_csv_mean_W + row.P2_mech_csv_mean_W;
        row.P_elec_csv_total_W = row.P1_elec_csv_mean_W + row.P2_elec_csv_mean_W;
...
function R = rotorStatsSingleRamp(t, data, idx, colT, colQ, colRPM, colV, colA, ...
                                  colPWM, colPmechCSV, colPelecCSV, ...
   ...
   t_win = t(idx);
    ...
    T_all_raw = getColumn(data, colT);
    RPM_all   = getColumn(data, colRPM);
    PWM_all   = getColumn(data, colPWM);
    
    if applyTareCorrection
    
        tare = estimateTareFromStartEnd( ...
            t, T_all_raw, RPM_all, PWM_all, ...
            tareWindow_s, tareGuard_s, ...
            tareRPMmax, tarePWMmax, tareUseEndZero);
    
        T_zero_all = interp1( ...
            tare.t_ref, tare.T_zero_kgf_ref, ...
            t, 'linear', 'extrap');
    
        T_kgf_raw = T_all_raw(idx) - T_zero_all(idx);
    
    else
    
        tare.T_zero_start_kgf = 0;
        tare.T_zero_end_kgf = 0;
        tare.T_zero_used_mode = "none";
        tare.N_zero_start = 0;
        tare.N_zero_end = 0;
        tare.t_ref = [t(1); t(end)];
        tare.T_zero_kgf_ref = [0; 0];
    ...
    P_mech_csv_raw = getColumn(data, colPmechCSV, idx);
    P_elec_csv_raw = getColumn(data, colPelecCSV, idx);
    
    % Salva diagnostica tara
    R.T_tare_correction_enabled = applyTareCorrection;
    R.T_zero_start_kgf = tare.T_zero_start_kgf;
    R.T_zero_end_kgf   = tare.T_zero_end_kgf;
    R.T_zero_start_N   = tare.T_zero_start_kgf * kgf2N;
    R.T_zero_end_N     = tare.T_zero_end_kgf   * kgf2N;
    R.N_zero_start     = tare.N_zero_start;
    R.N_zero_end       = tare.N_zero_end;
    R.T_zero_used_mode = tare.T_zero_used_mode;
    % ============================================================
    % Raw statistics
    % ============================================================
    ...
    P_mech_csv = P_mech_csv_raw(isfinite(P_mech_csv_raw));
    P_elec_csv = P_elec_csv_raw(isfinite(P_elec_csv_raw));
    
    R.N_P_mech_csv = numel(P_mech_csv);
    R.N_P_elec_csv = numel(P_elec_csv);
    
    R.P_mech_csv_mean_W = mean(P_mech_csv, 'omitnan');
    R.P_mech_csv_std_W  = std(P_mech_csv,  'omitnan');
    
    R.P_elec_csv_mean_W = mean(P_elec_csv, 'omitnan');
    R.P_elec_csv_std_W  = std(P_elec_csv,  'omitnan');
...
function row = writeRotorToRow(row, R, rotorID)
    ...
    % Tare / zero diagnostic
    row.(sprintf('T%s_tare_correction_enabled',s)) = R.T_tare_correction_enabled;
    row.(sprintf('T%s_zero_start_kgf',s)) = R.T_zero_start_kgf;
    row.(sprintf('T%s_zero_end_kgf',s))   = R.T_zero_end_kgf;
    row.(sprintf('T%s_zero_start_N',s))   = R.T_zero_start_N;
    row.(sprintf('T%s_zero_end_N',s))     = R.T_zero_end_N;
    row.(sprintf('N_T%s_zero_start',s))   = R.N_zero_start;
    row.(sprintf('N_T%s_zero_end',s))     = R.N_zero_end;
    row.(sprintf('T%s_zero_used_mode',s)) = string(R.T_zero_used_mode);
    ...
    % Potenze lette direttamente dal CSV
    row.(sprintf('P%s_mech_csv_mean_W',s)) = R.P_mech_csv_mean_W;
    row.(sprintf('P%s_mech_csv_std_W',s))  = R.P_mech_csv_std_W;
    row.(sprintf('N_P%s_mech_csv',s))      = R.N_P_mech_csv;
    
    row.(sprintf('P%s_elec_csv_mean_W',s)) = R.P_elec_csv_mean_W;
    row.(sprintf('P%s_elec_csv_std_W',s))  = R.P_elec_csv_std_W;
    row.(sprintf('N_P%s_elec_csv',s))      = R.N_P_elec_csv;
...
function row = writeRotorNaN(row, rotorID)
    ...
    row.(sprintf('T%s_tare_correction_enabled',s)) = false;
    row.(sprintf('T%s_zero_start_kgf',s)) = NaN;
    row.(sprintf('T%s_zero_end_kgf',s))   = NaN;
    row.(sprintf('T%s_zero_start_N',s))   = NaN;
    row.(sprintf('T%s_zero_end_N',s))     = NaN;
    row.(sprintf('N_T%s_zero_start',s))   = NaN;
    row.(sprintf('N_T%s_zero_end',s))     = NaN;
    row.(sprintf('T%s_zero_used_mode',s)) = "not_computed";
    ...
    row.(sprintf('P%s_mech_csv_mean_W',s)) = NaN;
    row.(sprintf('P%s_mech_csv_std_W',s))  = NaN;
    row.(sprintf('N_P%s_mech_csv',s))      = NaN;
    
    row.(sprintf('P%s_elec_csv_mean_W',s)) = NaN;
    row.(sprintf('P%s_elec_csv_std_W',s))  = NaN;
    row.(sprintf('N_P%s_elec_csv',s))      = NaN;
...
function tare = estimateTareFromStartEnd( ...
    t, T_raw, RPM, PWM, ...
    tareWindow_s, tareGuard_s, ...
    tareRPMmax, tarePWMmax, tareUseEndZero)
    ...
    % Finestra iniziale
    idxStartTime = t >= t0 + tareGuard_s & ...
                   t <= t0 + tareGuard_s + tareWindow_s;

    % Finestra finale
    idxEndTime = t >= t1 - tareGuard_s - tareWindow_s & ...
                 t <= t1 - tareGuard_s;

    % Per la tara uso i campioni di T validi nella finestra.
    % Non richiedo PWM/RPM validi nello stesso campione, perché questi canali
    % possono essere sparsi rispetto alla cella di carico.
    idxStart = idxStartTime & isfinite(T_raw);
    idxEnd   = idxEndTime   & isfinite(T_raw);

    T_zero_start = mean(T_raw(idxStart), 'omitnan');
    T_zero_end   = mean(T_raw(idxEnd),   'omitnan');

    N_start = sum(idxStart);
    N_end   = sum(idxEnd);

    t_start_ref = mean(t(idxStart), 'omitnan');
    t_end_ref   = mean(t(idxEnd),   'omitnan');

    % Diagnostica separata su PWM/RPM nelle finestre.
    PWM_start = PWM(idxStartTime & isfinite(PWM));
    PWM_end   = PWM(idxEndTime   & isfinite(PWM));

    RPM_start = RPM(idxStartTime & isfinite(RPM));
    RPM_end   = RPM(idxEndTime   & isfinite(RPM));

    fprintf('\n  TARE DEBUG corrected:\n');
    fprintf('    N T start = %d | N T end = %d\n', N_start, N_end);
    fprintf('    T zero start = %.8f kgf | T zero end = %.8f kgf\n', ...
        T_zero_start, T_zero_end);

    if ~isempty(PWM_start)
        fprintf('    PWM start finite min/mean/max = %.1f / %.1f / %.1f\n', ...
            min(PWM_start), mean(PWM_start), max(PWM_start));
    else
        fprintf('    PWM start finite samples: none\n');
    end

    if ~isempty(PWM_end)
        fprintf('    PWM end finite min/mean/max = %.1f / %.1f / %.1f\n', ...
            min(PWM_end), mean(PWM_end), max(PWM_end));
    else
        fprintf('    PWM end finite samples: none\n');
    end

    if ~isempty(RPM_start)
        fprintf('    RPM start finite min/mean/max = %.1f / %.1f / %.1f\n', ...
            min(RPM_start), mean(RPM_start), max(RPM_start));
    else
        fprintf('    RPM start finite samples: none\n');
    end

    if ~isempty(RPM_end)
        fprintf('    RPM end finite min/mean/max = %.1f / %.1f / %.1f\n', ...
            min(RPM_end), mean(RPM_end), max(RPM_end));
    else
        fprintf('    RPM end finite samples: none\n');
    end

    % Warning diagnostici, ma non blocco la tara se T è valida.
    if ~isempty(PWM_start) && mean(PWM_start, 'omitnan') > tarePWMmax
        warning('Start tare window has PWM above tarePWMmax.');
    end

    if ~isempty(PWM_end) && mean(PWM_end, 'omitnan') > tarePWMmax
        warning('End tare window has PWM above tarePWMmax.');
    end

    if ~isempty(RPM_start) && mean(RPM_start, 'omitnan') > tareRPMmax
        warning('Start tare window has RPM above tareRPMmax.');
    end

    if ~isempty(RPM_end) && mean(RPM_end, 'omitnan') > tareRPMmax
        warning('End tare window has RPM above tareRPMmax.');
    end

    if N_start > 0 && N_end > 0 && tareUseEndZero

        tare.T_zero_start_kgf = T_zero_start;
        tare.T_zero_end_kgf   = T_zero_end;

        tare.t_ref = [t_start_ref; t_end_ref];
        tare.T_zero_kgf_ref = [T_zero_start; T_zero_end];

        tare.T_zero_used_mode = "linear_start_end_T_window";

    elseif N_start > 0

        tare.T_zero_start_kgf = T_zero_start;
        tare.T_zero_end_kgf   = T_zero_start;

        tare.t_ref = [t0; t1];
        tare.T_zero_kgf_ref = [T_zero_start; T_zero_start];

        tare.T_zero_used_mode = "constant_start_T_window";

    elseif N_end > 0

        tare.T_zero_start_kgf = T_zero_end;
        tare.T_zero_end_kgf   = T_zero_end;

        tare.t_ref = [t0; t1];
        tare.T_zero_kgf_ref = [T_zero_end; T_zero_end];

        tare.T_zero_used_mode = "constant_end_T_window";

    else

        warning('No valid T samples found in tare windows. Tare correction not applied.');
        ...
        tare.T_zero_used_mode = "not_available";

    end

    tare.N_zero_start = N_start;
    tare.N_zero_end = N_end;
